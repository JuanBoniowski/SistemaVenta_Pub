...
namespace SistemaVenta.DAL.Implementacion
{
    public class VentaRepository : GenericRepository<Venta>, IVentaRepository
    {
        ...
        public async Task<Venta> Registrar(Venta entidad)
        {
            ...
            using (var transaction =_dbContext.Database.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        Producto product_encontrado = _dbContext.Productos.Where(p => p.IdProducto == dv.IdProducto).First();

                        product_encontrado.Stock = product_encontrado.Stock - dv.Cantidad;
                        _dbContext.Productos.Update(product_encontrado); 
                    }
        ...
        {
            List<DetalleVenta> listaResume = await _dbContext.DetalleVenta
